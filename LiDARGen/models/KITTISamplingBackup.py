...
@torch.no_grad()
def anneal_Langevin_dynamics_inpainting_simultaneous_basic_kitti(x_mod, refer_image, refer_mask, sky, x_indices, minStepToShare,setting,allowance, scorenet, sigmas, fromWorld, toWorld,
        ...
        # for c, sigma in enumerate(sigmas[:3]):
            ...
            for s in range(n_steps_each):
                ...
                if(c >= minStepToShare):
                    ...
                    pointArray = pointArray.reshape(4,-1)
                    ...
                    relativePoints = torch.bmm(fromWorld,bigCloud)[:,:3]
                    ...
                    for origin in range(batchSize):
                        ...
                        imageIntensity = torch.sparse_coo_tensor(merged,torch.flatten(x_mod[:,1,:,:])[final_ind], size=(bigRowCount,colMax)).to_dense()
                        ...
                        imageIntensityMin = torch.sparse_coo_tensor(mergedReduced,torch.flatten(x_mod[:,1,:,:])[final_ind], size=(bigRowCount,colMax)).to_dense()
                        ...
                        imageIntensity = torch.where(imageDepth > imageDepthMin + maxDifferenceAllowed, imageIntensityMin, imageIntensity)
                        #Meanwhile for average, don't let it be more than 10 metres shift
                        imageDepth = torch.where(imageDepth > imageDepthMin + maxDifferenceAllowed, imageDepthMin + maxDifferenceAllowed, imageDepth)
                        #And now relog it
