...
@torch.no_grad()
def anneal_Langevin_dynamics_inpainting_simultaneous_basic_kitti(x_mod, refer_image, refer_mask, sky, x_indices, minStepToShare,setting,allowance, scorenet, sigmas, fromWorld, toWorld, actualBatchSize,
    ...
    #Setting = 5:  have a min distance when making shared cloud
    #Setting = 6&7: increase correlation_coefficient as nears completion.
    # actualBatchSize = 2
    images = []
    targets = []
    sharedImages = []
    ...
    fromWorld = torch.squeeze(fromWorld).to(x_mod.device)
    toWorld = torch.squeeze(toWorld).to(x_mod.device)
    ...
    with torch.no_grad():
        ...
        # for c, sigma in enumerate(sigmas[:3]):
            ...
            # originList = ((torch.log2(torch.abs(originListOG)+1)) / 11) * heWhoModsTheModMan
            # originList = (torch.pow(2,(originList*11))-1)
            ...
            for s in range(n_steps_each):
                ...
                if(c >= minStepToShare):
                    ...
                    # print(originList[:batchSize,:,0])
                    pointX = (realDistance * torch.cos(azimuth).view(1,1,-1) * torch.cos(elevation).view(1,-1,1)).view(batchSize,-1)
                    pointY = (realDistance * torch.sin(azimuth).view(1,1,-1) * torch.cos(elevation).view(1,-1,1)).view(batchSize,-1)
                    pointZ = (realDistance * torch.sin(elevation).view(1,-1,1)).view(batchSize,-1)

                    #Now convert them to world view
                    pointArray = torch.stack((pointX,pointY,pointZ,torch.ones_like(pointX)),1)
                    # print("Z A SHAPES")
                    # print(toWorld.shape)
                    # print(pointArray.shape)
                    pointArray = torch.bmm(toWorld,pointArray)
                    # sharedCloud = torch.unsqueeze(torch.stack((pointX,pointY,pointZ,x_mod[:,1,:,:])).view(4,-1),0)
                    # sharedCloud = torch.unsqueeze(torch.stack((pointArray[:,0],pointArray[:,1],pointArray[:,2],x_mod[:,1,:,:].view(batchSize,-1))).view(4,-1),0)
                    #now I need to get the resultant distance for EVERY origin
                    pointArray = torch.movedim(pointArray,1,0)
                    pointArray = pointArray.reshape(4,int(batchSize/actualBatchSize),-1)
                    # bigCloud = torch.tile(torch.unsqueeze(pointArray,0),(batchSize,1,1))
                    bigCloud = torch.tile(torch.unsqueeze(pointArray,0),(actualBatchSize,1,1,1))
                    #This is now 8x4x8CH
                    #the 0 in originlist is necessary to reduce it from Ox3x1x1 to Ox3x1
                    #now it's time to go to each viewpoint
                    # relativePoints = torch.subtract(bigCloud,originList[:batchSize,:,0])
                    #big cloud is 8x3x8HW
                    #gotta make it 8x4x8HW
                    #Now it is 8x4x4x7HW, with first 4 being channels, second being which "megabatch" it is
                    relativePointsStack = []
                    ...
                    for currentMegaBatch in range(int(batchSize/actualBatchSize)):  
                        relativePointsStack.append(torch.bmm(fromWorld[(actualBatchSize*(currentMegaBatch)):(actualBatchSize*(currentMegaBatch+1))],bigCloud[:,:,currentMegaBatch])[:,:3])
                    relativePoints = torch.cat(relativePointsStack,0)
                    #And now it is 28x4x7HW which is what I want - 28 outputs, each considering points of 7 in total
                    ...
                    # inGrid = torch.logical_and(inGrid,torch.unsqueeze(sky[:actualBatchSize].flatten(),0))
                    ...
                    for origin in range(batchSize):
                        ...
                        #Ok so I need to be *slightly* smarter, in the sense that I need to remove background points... somehow?
                        #I have the minimum, I have the total, I have the average
                        #Lazy method is, if average is > Min + 5, set it to Min + 5?
                        #Let's just try that I guess
                    ...
                    # print(torch.max(torch.abs(x_mod[:,0,:,:]))*11)
            ...
            if verbose and c % 20 == 0 or c == 1 or c == 2:
...
@torch.no_grad()
def anneal_Langevin_dynamics_inpainting_simultaneous_second_method_kitti(x_mod, refer_image, refer_mask, sky, refer_indices, minStepToShare,setting, scorenet, sigmas,  fromWorld, toWorld,
    ...
    with torch.no_grad():
        ...
        # for c, sigma in enumerate(sigmas[:3]):
            ...
            for s in range(n_steps_each):
                ...
                isNeg = (x_mod[:,0,:,:] < 0).int()
                # tooHigh = torch.max(torch.abs(x_mod[:,0,:,:]))*11 > 50
                # print(torch.max(torch.abs(x_mod[:,0,:,:]))*11)
                #ok so this needs to handle negatives as well
                modifierArray = torch.ones_like(x_mod[:,0,:,:]) - (isNeg*2)
                sigmaMod = 1
        ...
        # #ok so this needs to handle negatives as well
        # modifierArray = torch.ones_like(x_mod[:,0,:,:]) - (isNeg*2)
        # realDistance = (torch.pow(2,(torch.abs(x_mod[:,0,:,:])*11))-1) * modifierArray
        # #for now just outright skip this when distances too large to overlap
        # #smart
        ...
        # # print(torch.min(realDistance))
        # # print(torch.max(newDepth))
        # newDepth = ((torch.log2(newDepth+1)) / 11)
        # newDepth = ((torch.log2(newDepth+1)) / 11)
        # # newDepth = newDepth / 2000
        # # print(torch.max(newDepth))
        # # newDepth = torch.clip(newDepth,min=0)
