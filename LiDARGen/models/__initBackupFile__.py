...
def anneal_Langevin_dynamics_inpainting_simultaneous_basic(x_mod, refer_image, refer_mask, sky, x_indices, minStepToShare,setting, scorenet, sigmas,
...
def anneal_Langevin_dynamics_inpainting_simultaneous_second_method(x_mod, refer_image, refer_mask, sky, x_indices, minStepToShare,setting, scorenet, sigmas,
                                        n_steps_each=100, step_lr=0.000008, denoise = True, verbose = True, grad_ref = 0.1, correlation_coefficient = 0.1, sampling_step = 16):
    ...
    raw = refer_image
    sky = sky.to(x_mod.device)
    x_indices = x_indices.to(x_mod.device)
    # raw_interp = torch.nn.functional.interpolate(raw, size=(64, 1024), mode='bilinear')
    # mask[:, :, 0:64:sampling_step, :] = 1
    step_refer = grad_ref# 0.001
    #for now make it the same as co-efficient to base input
    ...
    with torch.no_grad():
        ...
        # for c, sigma in enumerate(sigmas[:3]):
            ...
            for s in range(n_steps_each):
                ...
                # realNoise = (torch.pow(2,(torch.abs(totalGrad[:,0])*11))-1) * modifierArray
                ...
                if(c >= minStepToShare):
                    ...
                    noiseX = torch.add(realNoise * torch.cos(azimuth).view(1,1,-1) * torch.cos(elevation).view(1,-1,1),originList[:batchSize,0])
                    noiseY = torch.add(realNoise * torch.sin(azimuth).view(1,1,-1) * torch.cos(elevation).view(1,-1,1),originList[:batchSize,1])
                    noiseZ = torch.add(realNoise * torch.sin(elevation).view(1,-1,1),originList[:batchSize,2])
