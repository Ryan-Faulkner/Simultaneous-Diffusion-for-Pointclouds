...
class KITTI360_im_simultaneous_densification(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #For inpainting
        index[:, :(self.colMax//4)] = -2

        ogReal[:, :(self.colMax//4)] = 0

        ogIntensity[:, :(self.colMax//4)] =  0
        if(numberInBatch == 0):
        ...
        if(numberInBatch == 0):
            #COMMENT OUT IF YOU ONLY WANT DENSIFICATION AND NO INPAINTING WHATSOEVER
            mask = np.zeros_like(mask)
            #For just densification
            # mask = np.logical_or(mask,maskTwo)
            #For just Inpainting
            mask = np.logical_or(mask,maskThree)
