...
class KITTIGetMISSING(Dataset):
    ...
    def __init__(self, path, config, split = 'train', resolution=None, transform=None):
        ...
        self.full_list = glob(os.path.join('KITTI-360', 'data_3d_raw/randomDriveFiles/*.bin'))
        # if split == "train":
    ...
    def __getitem__(self, idx):
        ...
        if self.return_remission:
            real, intensity = point_cloud_to_range_image_OG(filename, False, self.return_remission)
        else:
            real = point_cloud_to_range_image_OG(filename, False, self.return_remission)
        #Make negatives 0
