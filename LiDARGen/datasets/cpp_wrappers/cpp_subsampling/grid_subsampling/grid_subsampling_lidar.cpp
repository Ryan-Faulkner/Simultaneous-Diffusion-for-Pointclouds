...
#include "grid_subsampling_lidar.h"
#include <cmath>
...
void grid_subsampling_lidar(vector<PointXYZ>& original_points,
	...
	for (auto& p : original_points)
	{
		...
		if(use_feature)
		{
			float gridX = (original_features.begin() + i * fdim)[-2];
			float gridY = (original_features.begin() + i * fdim)[-1];
			int best = 0;
			for (int m = 1; m < 17; m++)
			{
				if((int)gridX % (int)(pow(2,m)) and (int)gridY % (int)(pow(2,m)))
					best = m;
				else
					break;
			}
			if(data[mapIdx].highestMod < best)
			{
				if (use_classes)
					data[mapIdx].update_all(p, original_features.begin() + i * fdim, original_classes.begin() + i * ldim, best);
				else
					data[mapIdx].update_features(p, original_features.begin() + i * fdim, best);
			}
		} else
		{
			...
			if (use_feature && use_classes)
				data[mapIdx].update_all(p, original_features.begin() + i * fdim, original_classes.begin() + i * ldim,-1);
			else if (use_feature)
				data[mapIdx].update_features(p, original_features.begin() + i * fdim,-1);
			else if (use_classes)
	...
	for (auto& v : data)
	{
		...
		if (use_feature)
		{
		    float count = 1;//(float)v.second.count;
