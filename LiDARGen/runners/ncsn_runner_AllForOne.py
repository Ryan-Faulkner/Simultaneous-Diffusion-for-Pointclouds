...
class ncsn_runner_AllForOne():
    ...
    def sample(self):
        if False:#self.config.sampling.ckpt_id is None:
            states = torch.load(os.path.join(self.args.log_path, 'checkpoint.pth'), map_location=self.config.device)
        else:
        ...
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                ...
                # existVals = existVals > 7 #Get pixels which show up in less than 7/21 scans, aka one third.
                ...
                # for batchesToDo in range(26):
                    ...
                    refer_images_full,refer_mask_full, refer_sky, refer_indices_full, toWorld_full, fromWorld_full, goalImages, toOGView, saveNumArray = next(data_iter)
                    # if(batchesToDo < 4):
                    #     continue
                    saveNum = ""
                    ...
                    endPoint = self.config.sampling.actualBatchSize
                    toAdd = 0                    
                    if self.config.data.dataset == 'KITTI360_im_simultaneous_densification':
                        #No need for partical circle when doing densification, we aren't running ablation for that task
                        endPoint = 2
                        toAdd = self.config.sampling.actualBatchSize - 2
                    for doThis in range(endPoint):
                        ...
                        # if(doThis == 0):
                        #     save_image(image_grid, os.path.join(self.args.image_folder,
                        #                                         str(doThis) + '_' + saveNum + '_Mask_image_grid_{}.png'.format(self.config.sampling.ckpt_id)))
                        ...
                        if(doThis + toAdd == self.config.sampling.actualBatchSize - 1):
                            ...
                            all_outputs, all_targets = anneal_Langevin_dynamics_inpainting(init_samples, refer_images_for_diffusion, refer_mask ,score, sigmas,
                                                                ...
                                                                grad_ref=1,
                                                                sampling_step=4)
                        elif doThis + toAdd < self.config.sampling.actualBatchSize - 2:
                        # if True:
                            #remove unnecessary images
                            refer_images_for_diffusion = torch.reshape(refer_images_for_diffusion,(batchProportions,self.config.sampling.actualBatchSize,-1))
                            refer_images_for_diffusion = torch.reshape(refer_images_for_diffusion[:,:(doThis+2)],(batchProportions*(doThis+2),self.config.data.channels,self.config.data.image_size,self.config.data.image_width))

                            init_samples = torch.reshape(init_samples,(batchProportions,self.config.sampling.actualBatchSize,-1))
                            init_samples = torch.reshape(init_samples[:,:(doThis+2)],(batchProportions*(doThis+2),self.config.data.channels,self.config.data.image_size,self.config.data.image_width))

                            refer_mask = torch.reshape(refer_mask,(batchProportions,self.config.sampling.actualBatchSize,-1))
                            refer_mask = torch.reshape(refer_mask[:,:(doThis+2)],(batchProportions*(doThis+2),self.config.data.channels,self.config.data.image_size,self.config.data.image_width))

                            sky = torch.reshape(sky,(batchProportions,self.config.sampling.actualBatchSize,-1))
                            sky = torch.reshape(sky[:,:(doThis+2)],(batchProportions*(doThis+2),1,self.config.data.image_size,self.config.data.image_width))

                            refer_indices = torch.reshape(refer_indices,(batchProportions,self.config.sampling.actualBatchSize,-1))
                            refer_indices = torch.reshape(refer_indices[:,:(doThis+2)],(batchProportions*(doThis+2),1,self.config.data.image_size,self.config.data.image_width))

                            toWorld = torch.reshape(toWorld,(batchProportions,self.config.sampling.actualBatchSize,-1))
                            toWorld = torch.reshape(toWorld[:,:(doThis+2)],(batchProportions*(doThis+2),4,4))

                            fromWorld = torch.reshape(fromWorld,(batchProportions,self.config.sampling.actualBatchSize,-1))
                            fromWorld = torch.reshape(fromWorld[:,:(doThis+2)],(batchProportions*(doThis+2),4,4))

                            all_outputs, all_targets, all_shared = anneal_Langevin_dynamics_inpainting_simultaneous_basic(init_samples, refer_images_for_diffusion, refer_mask , sky,refer_indices, startStep, setting, score, sigmas, modifiersToGive, doThis + 2,
                                                                    self.config.sampling.n_steps_each,
                                                                    self.config.sampling.step_lr,
                                                                    existMask = existVals,
                                                                    denoise=self.config.sampling.denoise,
                        ...
                        if doThis + toAdd == self.config.sampling.actualBatchSize - 1:
                            ...
                            sample_full.append(all_outputs[-1].view((doThis + 2)*batchProportions, self.config.data.channels,
                                                          self.config.data.image_size,
                                                          self.config.data.image_width))
                            # shared_full.append(all_shared[-1].view(self.config.sampling.batch_size, self.config.data.channels,
                            #                               self.config.data.image_size,
                            #                               self.config.data.image_width))
                            shared_intial.append(all_outputs[-2].view((doThis + 2)*batchProportions, self.config.data.channels,
                                                          self.config.data.image_size,
                                                          self.config.data.image_width))
                        else:
                        ...
                        if not self.config.sampling.final_only:
                            ...
                            # image_grid = make_grid(sample, int(np.sqrt(self.config.sampling.batch_size)))
                            # save_image(image_grid, os.path.join(self.args.image_folder,
                            #                                     str(doThis) + '_' + saveNum + '_image_grid_{}.png'.format(self.config.sampling.ckpt_id)))
                            # np.save(os.path.join(self.args.image_folder,
                            #                                 str(doThis) + '_' + saveNum + '_completion_{}.pth'.format(self.config.sampling.ckpt_id)),sample.cpu().detach().numpy())
                            
                            if doThis + toAdd == self.config.sampling.actualBatchSize - 1:
                                image_grid = make_grid(maskedSample, int(np.sqrt((1)*batchProportions)))
                                save_image(image_grid, os.path.join(self.args.image_folder,
                                ...
                                image_grid = make_grid(maskedSample, int(np.sqrt((doThis + 2)*batchProportions)))
                                save_image(image_grid, os.path.join(self.args.image_folder,
                                                                    str(doThis) + '_' + saveNum + '_Masked_image_grid_{}.png'.format(self.config.sampling.ckpt_id)))
                                np.save(os.path.join(self.args.image_folder,
                                                                str(doThis) + '_' + saveNum + '_Masked_completion_{}.pth'.format(self.config.sampling.ckpt_id)),maskedSample.cpu().detach().numpy())
                            ...
                            # save_image(image_grid, os.path.join(self.args.image_folder,
                            #                                     str(doThis) + '_' + saveNum + '_Shared_image_grid_final{}.png'.format(self.config.sampling.ckpt_id)))
                            # np.save(os.path.join(self.args.image_folder,
                            #                                 str(doThis) + '_' + saveNum + '_Shared_completion_final{}.pth'.format(self.config.sampling.ckpt_id)),sample_shared.cpu().detach().numpy())
                            if doThis + toAdd == self.config.sampling.actualBatchSize - 1:
                                image_grid = make_grid(sample_initial, int(np.sqrt((1)*batchProportions)))
                                save_image(image_grid, os.path.join(self.args.image_folder,
                                                                    str(doThis) + '_' + saveNum + '_Shared_image_grid_initial{}.png'.format(self.config.sampling.ckpt_id)))
                                np.save(os.path.join(self.args.image_folder,
                                                                str(doThis) + '_' + saveNum + '_Shared_completion_initial{}.pth'.format(self.config.sampling.ckpt_id)),sample_initial.cpu().detach().numpy())
                            elif doThis + toAdd < self.config.sampling.actualBatchSize - 2:
                                image_grid = make_grid(sample_initial, int(np.sqrt((doThis + 2)*batchProportions)))
                                save_image(image_grid, os.path.join(self.args.image_folder,
                                                                    str(doThis) + '_' + saveNum + '_Shared_image_grid_initial{}.png'.format(self.config.sampling.ckpt_id)))
                                np.save(os.path.join(self.args.image_folder,
                                                                str(doThis) + '_' + saveNum + '_Shared_completion_initial{}.pth'.format(self.config.sampling.ckpt_id)),sample_initial.cpu().detach().numpy())
