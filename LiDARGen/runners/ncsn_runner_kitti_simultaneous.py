...
__all__ = ['NCSNRunnerKITTISimultaneous']
...
class NCSNRunnerKITTISimultaneous():
    ...
    def sample(self):
        ...
        # valSize = 8
        # valSize = len(glob(os.path.join("/data/KITTI-360", 'data_3d_raw/2013_05_28_drive_0000_sync/velodyne_points/data/*.bin')))
        ...
        #Because I predict forwards let's just not let it use the very end of the drive as a start point
        #Test set is large enough that not including the last ~35 scans out of 10,000+ won't matter and saves the headache of
        #the fact I simply don't have a groundtruth for what comes after those scans.
        ##Put another way - they simply are not part of the test set for this task.
        ...
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                ...
                #Get pixels which show up in less than 7/21 scans, aka one third.
                ...
                for batchesToDo in range(valSize):
                    ...
                    # continue
                    #Do their method once for all batch - then batch-1 times, increasing size by 1 each time from 2->batch
                    for doThis in range(self.config.sampling.actualBatchSize):
                        ...
                        if(doThis == 0):
                            save_image(image_grid, os.path.join(self.args.image_folder,
                                                                str(doThis) + '_' + str(batchesToDo) + '_Input_image_grid_{}.png'.format(self.config.sampling.ckpt_id)))
                        ...
                        print("--- %s san check ---" % (timeTaken[doThis]/(batchesToDo+1)))
                        if(doThis == self.config.sampling.actualBatchSize - 1):
                            #already have this
                            # continue
                            ...
                            all_outputs, all_targets = anneal_Langevin_dynamics_inpainting(init_samples, refer_images_for_diffusion, refer_mask ,score, sigmas,
                                                                ...
                                                                grad_ref=1,
                                                                sampling_step=4)
                        elif(doThis < self.config.sampling.actualBatchSize-1):
                            if doThis < self.config.sampling.actualBatchSize - 2:
                                ...
                                all_outputs, all_targets, all_shared = anneal_Langevin_dynamics_inpainting_simultaneous_basic_kitti(init_samples, refer_images_for_diffusion, refer_mask , sky,refer_indices, startStep, setting, allowance, score, sigmas, fromWorld, toWorld, doThis+2,
                                                                        self.config.sampling.n_steps_each,
                                                                        self.config.sampling.step_lr,
                                                                        existMask = existVals,
                                                                        denoise=self.config.sampling.denoise,
                                ...
                                all_outputs, all_targets, all_shared = anneal_Langevin_dynamics_inpainting_simultaneous_basic_kitti(init_samples, refer_images_for_diffusion, refer_mask , sky,refer_indices, startStep, setting, allowance, score, sigmas, fromWorld, toWorld, self.config.sampling.actualBatchSize,
                                                                    self.config.sampling.n_steps_each,
                                                                    self.config.sampling.step_lr,
                                                                    existMask = existVals,
                                                                    denoise=self.config.sampling.denoise,
